...
def _representative_prefixes(hwids: list[str], silicon_vendor: str) -> list[str]:
    """Up to N distinct VEN/DEV (or VID/PID) prefixes, most common first —
    a family's HWID set can be huge (AMD graphics); its silicon ids are few.
    Where the family's silicon vendor has known PCI VEN ids, prefixes from
    other vendors' silicon (bundled INFs) are dropped."""
    ...
    for h in hwids:
        for pat in (_PCI, _USB, _HDA):
            ...
            if m:
                ...
                counts[prefix] += 1
                break
    return [p for p, _ in counts.most_common(MAX_QUERIES_PER_FAMILY)]
